...
namespace EquipmentFailureAnalysis.Tests
{
    public class PprTests
    {
        ...
        [Fact]
        public void PprCellToTooltipConverter_ShouldGenerateCorrectTooltips()
        {
            // Arrange
            var converter = new PprCellToTooltipConverter();
            var item = new PprScheduleItem
            {
                ...
                InventoryNumber = "111",
                CommissionYear = "2020",
                MonthlyPlans = new string?[] { "ТО", null, "ТР", null, null, null, null, null, null, null, null, null },
                MonthlyCompletions = new bool[] { true, false, false, false, false, false, false, false, false, false, false, false }
            };

            // Act 1: Completed cell tooltip
            var valuesCompleted = new object?[] { item, DateTime.Today.Year };
            var tooltipCompleted = converter.Convert(valuesCompleted, typeof(string), "0", System.Globalization.CultureInfo.InvariantCulture) as string;

            // Act 2: Uncompleted cell in future year
            var valuesFuture = new object?[] { item, DateTime.Today.Year + 1 };
            var tooltipFuture = converter.Convert(valuesFuture, typeof(string), "2", System.Globalization.CultureInfo.InvariantCulture) as string;

            // Assert
            Assert.NotNull(tooltipCompleted);
            Assert.Contains("Выполнено: ТО", tooltipCompleted);
            Assert.Contains("(Нажмите для отмены)", tooltipCompleted);

            Assert.NotNull(tooltipFuture);
            Assert.Contains("Запланировано: ТР", tooltipFuture);
            Assert.Contains("(Нажмите для отметки о выполнении)", tooltipFuture);
        }
    }
}
